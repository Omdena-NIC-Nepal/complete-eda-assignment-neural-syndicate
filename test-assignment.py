...
class TestClimateEDA(unittest.TestCase):
    ...
    def test_required_libraries(self):
        ...
        for lib in required_libs:
            self.assertIn(
                f"import {lib}", self.all_code, f"Missing required import for {lib}"
            )

    def test_data_loading(self):
        """Test that climate data is loaded"""
        self.assertIn(
            "read_csv('data/Climate_Change_Indicators.csv')",
            self.all_code,
    ...
    def test_yearly_aggregation(self):
        ...
        found_yearly_agg = any(
            re.search(pattern, self.all_code) for pattern in yearly_agg_patterns
        )
    ...
    def test_univariate_analysis(self):
        ...
        found_univariate_vis = any(
            re.search(pattern, self.all_code) for pattern in univariate_vis_patterns
        )
        self.assertTrue(
            found_univariate_vis, "No evidence of univariate visualizations"
        )
        ...
        found_stats = any(
            re.search(pattern, self.all_code) for pattern in stats_patterns
        )
        self.assertTrue(
            found_stats, "No evidence of descriptive statistics calculation"
        )
    ...
    def test_bivariate_analysis(self):
        ...
        found_bivariate_vis = any(
            re.search(pattern, self.all_code) for pattern in bivariate_vis_patterns
        )
    ...
    def test_multivariate_analysis(self):
        ...
        found_multivariate_vis = any(
            re.search(pattern, self.all_code) for pattern in multivariate_vis_patterns
        )
        self.assertTrue(
            found_multivariate_vis, "No evidence of multivariate visualizations"
        )
    ...
    def test_conclusions_present(self):
        ...
        found_conclusion = any(
            re.search(pattern, self.all_markdown) for pattern in conclusion_patterns
        )
        self.assertTrue(
            found_conclusion, "No evidence of conclusions or insights in the analysis"
        )
    ...
    def test_min_number_of_visualizations(self):
        ...
        num_vis = sum(
            len(re.findall(pattern, self.all_code)) for pattern in vis_function_patterns
        )
        self.assertGreaterEqual(
            num_vis, 5, "Insufficient number of visualizations (minimum 5 required)"
        )
